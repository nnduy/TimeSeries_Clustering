...
def get_avd_3genres_3locations(df_3genres, df_3genres_3locations, df_avd):
    ...
    # air_visit_data.csv
    print("Full dataframe of air visit data rows - df rows:", df_avd.shape[0])
    if SPLIT_GROUPS == 3:
        df = df_3genres
    else: # SPLIT_GROUPS == 9
...
def get_hvd_3genres_3locations(df_3genres, df_3genres_3locations, df_hr_format):
    ...
    print("====================================================")
    print("Dataframe of hpg visit data from chosen time series:", df_ts_visits.shape[0])
    print("====================================================")
...
def average_first_last_moments(fm, lm):
    # if res == "hpg":
    #     format_datetime_df(fm)
    #     format_datetime_df(lm)
...
average_first_moment, average_last_moment = average_first_last_moments(first_moment, last_moment)
...
def imputing_one_timeseries(df, sid, dr_idx, method, column, j):
    ...
    if method == 'mean':
        ...
        # interpolate - linear values imputation method
        # Please note that only method='linear' is supported for DataFrame/Series with a MultiIndex.
        series['visitors'] = series['visitors'].interpolate(method=method, inplace=True)
        # df['visitors'].interpolate(method='slinear', inplace=True)
...
def imputing_all_timeseries(df_store_and_visit, dr_idx, method, floor_percentage):
    ...
    print("maximum_num_values_of_all_remained_series:", maximum_num_values_of_all_remained_series)
...
date_range_idx = pd.date_range(average_first_moment, average_last_moment)
...
visitor_df = imputing_all_timeseries(df_store_and_visit, date_range_idx,
                               IMPUTATION_METHOD[0], MAX_MISSING_PERCENTAGE[0])
print('visitor_df head  :\n {} \n'.format(visitor_df.head()))
print('visitor_df tail  :\n {} \n'.format(visitor_df.tail()))
...
#
#
# # ============ Step 07: Clustering using OPTICS algorithms ==============
#
#
...
# # ============ Step 08: Clustering using Hierachy algorithms ==============
...
def df_dendogram(res, data, visitor_matrix):
    ...
    print("visitor_matrix:\n", visitor_matrix)
    # print("list(visitor_matrix.columns.values):", list(visitor_matrix.columns.values))
    visitor_matrix.index.name = store_id
    visitor_matrix = visitor_matrix.reset_index()
    # print("list(visitor_matrix.columns.values):", list(visitor_matrix.columns.values))
    print("df_dendogram - visitor_matrix: ---- \n", visitor_matrix)


    df_id = visitor_matrix
    # df_id = df_id.to_frame()
    # print("df_dendogram - df_id:\n", df_id)
    # del visitor_matrix['Unnamed: 0'], which is the first column name of store id. It will be used for input
    X = visitor_matrix.iloc[:, 1:].values
    print("df_dendogram - Stacking all timeseries and use it as distance matrix:\n", X)
    ...
    #     df_merge_id = pd.merge(df_store_id, df_id, how='inner', on=[store_id])
    # else:
    #     print("df_dendogram - Not sufficient restaurant store chain")

    print("df_dendogram - df_merge_id:\n", df_merge_id)
...
def labeling_hierachy_cluster(dataset_ts_arg, affinity_arg, linkage_arg):
    ...
    hc = AgglomerativeClustering(n_clusters = NUM_OF_CLUSTER, affinity = affinity_arg, linkage =linkage_arg)
...
def main_labelling_clustering(df, lables):
...
def clustering_by_hierachy_main(dataset_ts, df):
    ...
    temp_df = df
    print("temp_df before :============== \n", temp_df.head())
    temp_df = temp_df[[store_id, genre_name, area_name]]
    print("temp_df after  :============== \n", temp_df.head())
    df_genre_clusters = temp_df.groupby([genre_name]).size().reset_index()
    ...
    for lnk in linkage_list:
        ...
        for aff in affinity_list:
            lables = labeling_hierachy_cluster(dataset_ts, aff, lnk)
            main_labels_df = main_labelling_clustering(temp_df, lables)
...
dataset_ts, df_merge_id = df_dendogram(res, data, visitor_df_transposed)
clustering_by_hierachy_main(dataset_ts, df_merge_id)
